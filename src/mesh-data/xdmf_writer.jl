...
- Supported types: :rectilinear, :structured, :unstructured
...
function write_xdmf(xdmf_filename::String, data_filename::String, mesh_filename::String, mesh; data_name="solution", times=nothing)
    ...
    elseif mesh_type == :structured
        ...
        dims = "$(length(mesh[:z])) $(length(mesh[:y])) $(length(mesh[:x]))"
        geom_xml = """
        <DataItem Dimensions=\"$(length(mesh[:x])) $(length(mesh[:y])) $(length(mesh[:z])) 3\" NumberType=\"Float\" Precision=\"8\" Format=\"HDF\">$mesh_filename:/coords</DataItem>
        """
    elseif mesh_type == :unstructured
        topology = mesh[:topology]  # e.g., "Polyvertex", "Hexahedron", etc.
        geometry = "XYZ"
    ...
    if times !== nothing && length(times) > 0
        ...
        for ti in 1:ntime
            grid = string(
                ...
                "            ", ti-1, " 0 0 0\n",
                "            1 1 1 1\n",
                "            1 ", length(mesh[:z]), " ", length(mesh[:y]), " ", length(mesh[:x]), "\n",
                "          </DataItem>\n",
                "          <DataItem Dimensions=\"$ntime ", length(mesh[:z]), " ", length(mesh[:y]), " ", length(mesh[:x]), "\" NumberType=\"Float\" Precision=\"8\" Format=\"HDF\">$data_filename:/$data_name</DataItem>\n",
                "        </DataItem>\n",
                "      </Attribute>\n",
                "    </Grid>\n"
            )
